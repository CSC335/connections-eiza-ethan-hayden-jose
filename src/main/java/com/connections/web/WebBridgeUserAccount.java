...
public class WebBridgeUserAccount extends WebBridgeUser implements ModularWeb, DatabaseFormattable, DatabaseUnique {
	...
	public static List<WebBridgeUserAccount> getAllAccounts(WebContext webContext) {
		MongoCollection<Document> collection = webContext.getMongoDatabase().getCollection(WebBridge.COLLECTION_ACCOUNT);
	...
	public Document getAsDatabaseFormat() {
		...
		doc.append(KEY_PASSWORD, passWord);
	...
	public void loadFromDatabaseFormat(Document doc) {
		...
		passWord = doc.getString(KEY_PASSWORD);
